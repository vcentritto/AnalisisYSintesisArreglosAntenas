...
% ¿CUÁLES MÓDULOS USAN LA FUNCIÓN fAnchoHaz3DBPlanoXY?
...
% ¿QUÉ SE PUEDE ENCONTRAR EN LA FUNCIÓN fAnchoHaz3DBPlanoXY?
...
%     yLobPrin ------------------------> Posición del lóbulo principal (en phi = eje "y" de MATLAB)
%     xLobPrin ------------------------> Posición del lóbulo principal (en theta = eje "x" de MATLAB)
...
figure('Name','Corte del diagrama de radiación en phi0 (Ancho del haz, máximos principal y nulos)');
...
% I.6. Gráfica del corte del diagrama de radiación -> incluye mínimos y ancho de haz a - 3 dB
...
cad1Phi = ['{\bf\fontsize{20} Corte del diagrama en \phi_{0} = ', num2str(yLobPrin), 'º}'];
cad2Phi = '{\it\fontsize{14} Ancho del haz, máximo principal y nulos}';
len_diff = length(cad1Phi) - length(cad2Phi);
if len_diff > 0
    cad2Phi = [cad2Phi, blanks(len_diff)];
elseif len_diff < 0
    cad1Phi = [cad1Phi, blanks(-len_diff)];
end
title({cad1Phi; cad2Phi}, 'FontName', 'Times New Roman','FontWeight','Normal');
...
if xLobPrin > -2 && xLobPrin < 2
    ...
    mDesfasajeAmbosParaTheta = 0;
    posicionMinDesfasajeAmbosParaThetaPositivo = 0;
    anchoHazDesfasajeAmbosParaTheta = 0;
    
else
    figure('Name','Corte del diagrama de radiación en theta0 (Ancho de haz, máximos y nulos)');
    ...
    % Condición que ocurre con cuatro elementos -> encuentra un solo mínimo o ninguno
    if length(ordenCasoTheta) == 1
        ...
        %Título y subtítulo concatenado
        cad1para4elementos = ['{\bf\fontsize{20} Corte del diagrama en \theta_{0} = ', num2str(xLobPrin), 'º}'];
        cad2para4elementos = '{\it\fontsize{14} Ancho del haz, máximo principal y nulos}';
        len_diff = length(cad1para4elementos) - length(cad2para4elementos);
        if len_diff > 0
            cad2para4elementos = [cad2para4elementos, blanks(len_diff)];
        elseif len_diff < 0
            cad1para4elementos = [cad1para4elementos, blanks(-len_diff)];
        end
        title({cad1para4elementos; cad2para4elementos}, 'FontName', 'Times New Roman','FontWeight','Normal');
        %Etiquetas de los ejes
        ...
        % II.6. Gráfica del corte del diagrama de radiación -> incluye mínimos y ancho de haz a - 3 dB
        hold on
        grid minor   
        % - Aplica condición única de ancho de haz a - 3dB
        if anchoDeHaz3dBParaTheta == 0
        else 
            %Se realiza línea de haz a -3 dB
        ...
        set(ax,'FontName', 'Times New Roman','FontSize', 15);
        %Título y subtítulo concatenado
        cad1Theta = ['{\bf\fontsize{20} Corte del diagrama en \theta_{0} = ', num2str(xLobPrin), 'º}'];
        cad2Theta = '{\it\fontsize{14} Ancho del haz, máximo principal y nulos}';
        len_diff = length(cad1Theta) - length(cad2Theta);
        if len_diff > 0
            cad2Theta = [cad2Theta, blanks(len_diff)];
        elseif len_diff < 0
            cad1Theta = [cad1Theta, blanks(-len_diff)];
        end
        title({cad1Theta; cad2Theta}, 'FontName', 'Times New Roman','FontWeight','Normal');
        %Etiquetas de los ejes
        xlabel("\phi",'FontSize', 16,'FontWeight', 'bold','FontName', 'Times New Roman');
        xtickformat('degrees');
        ylabel("|AF|",'FontSize', 16,'FontWeight', 'bold','FontName', 'Times New Roman');
