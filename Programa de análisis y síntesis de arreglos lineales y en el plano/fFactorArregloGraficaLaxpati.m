...
for numNulo = 1:1:cantNulos
    [imprimirExc] = fSimbolosLaxpati(vectorExcitaciones2,vectorFases,numNulo,cantNulos);
    disp(imprimirExc);
end
...
% GRAFICA ESFERICA
fGraficaEsfericaLinealLaxpati(theta,phi,fcGrafica,colorLaxpati);
...
if CN == 3    
    for numGrafica = 1:1:CN
        %Se hacen los cálculos para la gráfica
...
elseif CN == 4
    for numGrafica = 1:1:CN
        %Se realizan cálculos para la gráfica
        Arg1 = j*kd1*sin(theta)*cos((nulos(numGrafica,2)*pi/180)-fi1);
        Arg2 = j*kd2*sin(theta)*cos((nulos(numGrafica,2)*pi/180)-fi2);
        fc1 = excTotal(1,1)*exp(Arg1)+excTotal(1,2)*exp(Arg2)+excTotal(1,3)*exp(-Arg1)+excTotal(1,4)*exp(-Arg2);
        fc1 = fc1 / max(fc1);
...
elseif CN == 6
    for numGrafica = 1:1:CN
        %Se realizan cálculos para graficar
        Arg1 = j*kd1*sin(theta)*cos((nulos(numGrafica,2)*pi/180)-fi1);
        Arg2 = j*kd2*sin(theta)*cos((nulos(numGrafica,2)*pi/180)-fi2);
        fc1 = excTotal(1,1)*exp(Arg1)+excTotal(1,2)*exp(Arg2)+excTotal(1,3)*exp(-Arg1)+excTotal(1,4)*exp(-Arg2);
        fc1 = fc1 / max(fc1);
        ...
        for variar = 1:1:315
            valorIter = valoresX(variar);
            if valorIter > nulos(numGrafica,1)
                %Se encontró nulo para graficar
                posicion = variar-1;
                valorZ1 = abs(valoresZ(variar));
                valorZ2 = abs(valoresZ(posicion));
                if valorZ1 > valorZ2
...
elseif CN == 21  
    for numGrafica = 1:1:CN
        ...
        for variar = 1:1:315
            valorIter = valoresX(variar);
            if valorIter > nulos(numGrafica,1)
                %ESe encontró nulo para graficar
                posicion = variar-1;
                valorZ1 = abs(valoresZ(variar));
                valorZ2 = abs(valoresZ(posicion));
                if valorZ1 > valorZ2
...
elseif CN == 24
   ...
   disp('Los datos ingresados presentan un error. Intente de nuevo') 
end