function [ejeZ,ejeX,ejeY] = fGraficayAnalisisDolphChebyshevMejorado(ejeZ,ejeX,ejeY,ejeZCorte,titaCorte,N,pTheta,pPhi,d,angulocorteTheta,angulocortePhi,nivelRelativoAdim,directividad)
...
%     ejeZCorte -----------------------> Valores que toma el factor del arreglo cuando phi = 0°.
...
%Gráfica corte -> para observar posición del lóbulo principal y NRLL
figure('Name','Corte del diagrama en phi0 (NRLL y lóbulo principal en theta0');
ax = polaraxes;
%Se realizan gráficas
polarplot(ax,titaCorte,20*log10(ejeZCorte),'Color',colorcurva3dBChebyshev,'Linewidth',1.35);
set(ax,'FontName', 'Times New Roman','FontSize', 16);
hold on 
tita0 = linspace(-pi/2,pi/2); 
rho0 = (nivelLL)*ones(size(tita0)); 
polarplot(tita0,rho0,'--','Color', colormaximosChebyshev,'Linewidth',1.25);
hold off
%Características de ejes
thetalim([-90 90])
...
cad1Esferica = ['{\bf\fontsize{18} Corte del diagrama en \phi_{0} = ', num2str(pPhi), 'º}'];
cad2Esferica = '{\it\fontsize{14} NRLL y Lóbulo Principal en \theta_{0}}';
len_diff = length(cad1Esferica) - length(cad2Esferica);
if len_diff > 0
    cad2Esferica = [cad2Esferica, blanks(len_diff)];
elseif len_diff < 0
    cad1Esferica = [cad1Esferica, blanks(-len_diff)];
end
title({cad1Esferica; cad2Esferica}, 'FontName', 'Times New Roman','FontWeight','Normal');
%Condiciones para text de NRLL
if nivelLL >= -15
    text(deg2rad(ax.ThetaTick(3)),ax.RTick(2)*0.3,['NRRL = ',num2str(nivelLL),' dB'],'HorizontalAlignment','center','Rotation',75,'FontSize', 14,'Color', colormaximosChebyshev,'FontWeight', 'bold','FontName', 'Times New Roman');
elseif nivelLL <= -15
    text(deg2rad(ax.ThetaTick(3)),ax.RTick(2)*0.7,['NRRL = ',num2str(nivelLL),' dB'],'HorizontalAlignment','center','Rotation',75,'FontSize', 14,'Color', colormaximosChebyshev,'FontWeight', 'bold','FontName', 'Times New Roman');
end
...
figure('Name','Corte del diagrama de radiación en phi0 (NRLL)');
v = [0.5,0.5];
hold on
grid minor
ax = gca;
...
%Título y subtítulo concatenado
cad1Logaritmica = ['{\bf\fontsize{20} Corte del diagrama en \phi_{0} = ', num2str(pPhi), 'º}'];
cad2Logaritmica = '{\it\fontsize{14} Nivel relativo de los lóbulos laterales}';
len_diff = length(cad1Logaritmica) - length(cad2Logaritmica);
if len_diff > 0
    cad2Logaritmica = [cad2Logaritmica, blanks(len_diff)];
elseif len_diff < 0
    cad1Logaritmica = [cad1Logaritmica, blanks(-len_diff)];
end
title({cad1Logaritmica; cad2Logaritmica}, 'FontName', 'Times New Roman','FontWeight','Normal');
%Etiquetas de los ejes
xlabel("\theta",'FontSize', 16,'FontWeight', 'bold','FontName', 'Times New Roman');
ylabel("AF_{(dB)}",'FontSize', 16,'FontWeight', 'bold','FontName', 'Times New Roman');
xtickformat('degrees');
xticks(-90:10:90);
ylim([-70 0]);
hold off
...
disp([' El lóbulo principal se encuentra en: (',num2str(theta),num2str(numero0),' = ',num2str(pTheta),'°,',num2str(phi),num2str(numero0),' = ',num2str(pPhi),'°,|AF|',num2str(numero0),' = 1)']);
...
disp([num2str(rombos),num2str(rombos),' CORTE DEL DIAGRAMA EN ',num2str(phi),num2str(numero0),' = ',num2str(pPhi),'° ',num2str(flecha1),' ANCHO DE HAZ PRINCIPAL, ANCHO DE HAZ A - 3 DB y NULOS EN EL CORTE ',num2str(rombos),num2str(rombos)]);
disp([' ',num2str(flecha1),' A continuación, los resultados del ancho de haz principal, ancho a - 3 dB y nulos del corte en ',num2str(phi),num2str(numero0),' = ',num2str(pPhi),'° ']);
disp(' ');
...
if anchoHazDesfasajeAmbosParaPhi == 0
    ...
    disp(['        ',num2str(flecha2),' Posiblemente, el lóbulo principal no se encuentra dentro del intervalo: (-90°<',num2str(theta),'<90°).']);
else
...
if anchoDeHaz3dBParaPhi == 0
    ...
    disp(['        ',num2str(flecha2),' El lóbulo principal no se encuentra dentro del intervalo: (-90°<',num2str(theta),'<90°).']);
else
...
if pTheta == 0
    ...
    disp([num2str(rombos),num2str(rombos),' CORTE DEL DIAGRAMA EN ',num2str(theta),num2str(numero0),' = ',num2str(pTheta),'° ',num2str(flecha1),' ANCHO DE HAZ PRINCIPAL, ANCHO DE HAZ A - 3 DB y NULOS EN EL CORTE ',num2str(rombos),num2str(rombos)]);
    disp([' ',num2str(flecha1),' A continuación, los resultados del ancho de haz principal, ancho a - 3 dB y nulos del corte en ',num2str(theta),num2str(numero0),' = ',num2str(pPhi),'° ']);
    disp(' ');
    % - Ancho de haz principal -> se aplican ciertas condiciones
    if anchoHazDesfasajeAmbosParaTheta == 0
        disp(['     ',num2str(circulo),' No se pudo calcular el ancho principal']);
        disp(['        ',num2str(flecha2),' Posiblemente, el lóbulo principal no se encuentra dentro del intervalo: (-90°<',num2str(phi),'<90°).']);
    else
    ...
    if anchoDeHaz3dBParaTheta == 0
        disp(['     ',num2str(circulo),' No se pudo calcular el ancho de haz a - 3 dB.']);
        disp(['        ',num2str(flecha2),' El lóbulo principal no se encuentra dentro del intervalo: (-90°<',num2str(phi),'<90°).']);
    else
...
if angulocortePhi == 100 && angulocorteTheta == 100
    disp(['     ',num2str(circulo),' No se solicitaron otros cortes del diagrama de radiación (ni en ',num2str(theta),' o ',num2str(phi),')']);
