...
set(pax,'FontName', 'Times New Roman','FontSize', 16);
thetaticks(0:10:180)
thetalim([0 180]);
rlim([0 1]);
rticks(0:0.2:1)
...
elseif casoSintesisChebyshev == 2
    ...
    grid on                         
    grid minor
    title({'{\bf\fontsize{22} Diagrama de Radiación}'; '{\it\fontsize{16} Nivel relativo de lóbulos laterales}'},'FontName', 'Times New Roman','FontWeight','Normal')
%     title("Diagrama de Radiación",'FontSize', 22,'FontName', 'Times New Roman');
    xlabel("\theta",'FontSize', 16,'FontWeight','bold','FontName', 'Times New Roman');
    xtickformat('degrees')
    ylabel("AF_{(dB)}",'FontSize', 16,'FontWeight','bold','FontName', 'Times New Roman');
...
grid on                         
grid minor
% title({'{\bf\fontsize{22} Diagrama de Radiación}'; '{\it\fontsize{18} distancia inter-elemento = 0.5\lambda}'},'FontName', 'Times New Roman')
title("Diagrama de Radiación ",'FontSize', 22,'FontName', 'Times New Roman');
xlabel("\theta",'FontSize', 16,'FontWeight','bold','FontName', 'Times New Roman');
xtickformat('degrees')
...
if (((primerMinimoParaTheta > posicionLobuloPrincipal)&&(segundoMinimoParaTheta > posicionLobuloPrincipal))||((primerMinimoParaTheta < posicionLobuloPrincipal)&&(segundoMinimoParaTheta < posicionLobuloPrincipal)))
    disp([' No se puede calcular ancho de haz principal ',num2str(flecha1),' posiblemente, el lóbulo principal no se encuentra dentro del intervalo: (0°<',num2str(theta),'<180°).']);
else
...
if (((ladoDerecho > posicionLobuloPrincipal)&&(ladoIzquierdo > posicionLobuloPrincipal))||((ladoDerecho < posicionLobuloPrincipal)&&(ladoIzquierdo < posicionLobuloPrincipal)))
    disp([' No se puede calcular ancho de haz a -3 dB ',num2str(flecha1),' posiblemente, el lóbulo principal no se encuentra dentro del intervalo: (0°<',num2str(theta),'<180°).']);
else
