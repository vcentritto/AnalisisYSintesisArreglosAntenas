function [ejeZ,ejeX,ejeY] = fGraficayAnalisisPlano(ejeZ,ejeX,ejeY,ejeZCorte,titaCorte,N,anguloX,anguloY,pTheta,pPhi,d,angulocorteTheta,angulocortePhi,casoAnalisisoSintesis,verif,directividad)
...
%Gráfica corte -> para observar posición del lóbulo principal 
if anguloX == 0
else
    figure('Name','Corte del diagrama en phi0 (Lóbulo principal en theta0');
    ax = polaraxes;
    %Se realiza gráfica
    polarplot(ax,titaCorte,20*log10(ejeZCorte),'Color',colorcurva3dB,'Linewidth',1.35);
...
disp([' El lóbulo principal se encuentra en: (',num2str(theta),num2str(numero0),' = ',num2str(xLobPrin),'°,',num2str(phi),num2str(numero0),' = ',num2str(yLobPrin),'°,|AF|',num2str(numero0),' = ',num2str(zLobPrin),')']);
disp(' ');
...
if casoAnalisisoSintesis ==1
   ...
   %Movimiento en Theta
    ...
    if xLobPrin < 0
        ...
        disp([' El lóbulo principal se movió ',num2str(movimientoThetaCSigno),'° en ',num2str(theta),'.']);
        disp(' ');
    else
        disp([num2str(rombos),num2str(rombos),' ¿CUÁNTO SE MOVIÓ EL LÓBULO PRINCIPAL EN ',num2str(theta),'?',num2str(rombos),num2str(rombos)]);
        disp([' El lóbulo principal se movió ',num2str(movimientoTheta),'° en ',num2str(theta),'.']);
        disp(' ');
    end
    %Movimiento en Phi
    ...
    disp([' El lóbulo principal se movió ',num2str(movimientoPhi),'° en ',num2str(phi),'.']);
    disp(' ');
...
disp([num2str(rombos),num2str(rombos),' CORTE DEL DIAGRAMA EN ',num2str(phi),num2str(numero0),' = ',num2str(yLobPrin),'° ',num2str(flecha1),' ANCHO DE HAZ PRINCIPAL, ANCHO DE HAZ A - 3 DB y NULOS EN EL CORTE ',num2str(rombos),num2str(rombos)]);
disp([' ',num2str(flecha1),' A continuación, los resultados del ancho de haz principal, ancho a - 3 dB y nulos del corte en ',num2str(phi),num2str(numero0),' = ',num2str(yLobPrin),'° ']);
disp(' ');
...
if xLobPrin > -2 && xLobPrin < 2
else
    disp([num2str(rombos),num2str(rombos),' CORTE DEL DIAGRAMA EN ',num2str(theta),num2str(numero0),' = ',num2str(xLobPrin),'° ',num2str(flecha1),' ANCHO DE HAZ PRINCIPAL, ANCHO DE HAZ A - 3 DB y NULOS EN EL CORTE ',num2str(rombos),num2str(rombos)]);
    disp([' ',num2str(flecha1),' A continuación, los resultados del ancho de haz principal, ancho a - 3 dB y nulos del corte en ',num2str(theta),num2str(numero0),' = ',num2str(xLobPrin),'° ']);
    disp(' ');
    % - Ancho de haz principal -> se aplican ciertas condiciones
